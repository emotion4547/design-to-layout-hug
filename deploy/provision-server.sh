...
server {
    ...
    location / {
        ...
        # Последним идёт fallback.html, а не index.html: иначе на любой
        # несуществующий адрес отдавалась бы готовая разметка главной, и робот
        # видел бы её содержимое по десятку чужих адресов. У fallback.html та
        # же оболочка приложения, но с пометкой noindex.
        try_files $uri $uri/index.html $uri/ /fallback.html;
